...
export async function POST(request: NextRequest) {
  try {
    ...
    await prisma.voteChoice.deleteMany({ where: { vote: { pollId: { in: pollIds } } } });
    await prisma.vote.deleteMany({ where: { pollId: { in: pollIds } } });
    await prisma.slot.deleteMany({ where: { pollId: { in: pollIds } } });
    await prisma.cronSchedule.deleteMany({ where: { pollId: { in: pollIds } } });
    await prisma.poll.deleteMany({ where: { id: { in: pollIds } } });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.log(err);
    return NextResponse.json({ message: "Le sondage n'a pas pu être supprimé" }, { status: 500 });
  }
}
