...
export async function GET() {
    try {
        const cronSchedules = await prisma.cronSchedule.findMany({
            ...
            select: cronScheduleInclude,
        });

        // if no cron schedules, return
        if (cronSchedules.length === 0) return NextResponse.json({ message: 'No cron schedules found' });

        const pollsDone: string[] = [];
        ...
        return NextResponse.json({ message: 'Cron schedules done' });
    } catch (err) {
        console.log(err);
    }
}
...
const cronScheduleInclude = Prisma.validator<Prisma.CronScheduleSelect>()({
    id: true,
    poll: {
        select: {
            id: true,
...
type CronSchedule = Prisma.CronScheduleGetPayload<{ select: typeof cronScheduleInclude }>;
type Poll = CronSchedule['poll'];
