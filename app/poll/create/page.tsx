...
export default function CreatePoll() {
  ...
  const submitPoll = handleSubmit(async (data) => {
    ...
    const slots = data.slots.map((slot) => {
      ...
      const endDateTime = new Date(endDate.setHours(+endHours, +endMinutes, 0, 0));
      const utcEndSlot = fromZonedTime(endDateTime, 'Europe/Paris');

      return { maxParticipants: slot.maxParticipants, startDate: utcSlot, endDate: utcEndSlot };
    });
            ...
            <div key={field.id} className="flex flex-1 items-center flex-col justify-end border p-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Controller name={`slots.${index}.startTime`} control={control} render={({ field }) => <Input type="time" className="w-fit" {...field} />} />
                </div>
                <Label htmlFor="end">Date de fin</Label>
                <div className="flex flex-wrap gap-2">
                  <DatePicker control={control} name={`slots.${index}.endDate`} />
                  <Controller name={`slots.${index}.endTime`} control={control} render={({ field }) => <Input type="time" className="w-fit" {...field} />} />
                </div>
                {type == '2' && (
                  <>
